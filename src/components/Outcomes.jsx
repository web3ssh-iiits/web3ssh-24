    ...
    return (
        <>
            {(window.innerWidth >= 768) && (
                <section className="flex w-screen mx-auto min-h-screen bg-gray-950/40 text-gray-300 overflow-hidden" ref={container}>
                    ...
                    <div className='w-1/2 my-[100px]'>
                        {data.map((content, index) => (
                            <div key={index} className="group min-h-[300px] flex w-full items-center justify-start gap-4 content-div py-4 px-8 border-y text-2xl backdrop-blur-sm">
                                {content.graphics && (
                                    <div
                                        ...
                                        <img src={content.graphics} alt={content.title} className=" w-[150px] h-[150px] " />
                                    </div>
                                )}
                ...
                <section ref={horContainer} className="flex w-screen min-h-screen bg-gray-950/40 text-gray-300 flex-col border-b-2">
                    ...
                    <div className='w-full px-[100px] flex h-[calc(100vh-200px)] overflow-x-hidden'>
                        <div ref={horText} className='flex h-full'>
                            {data.map((content, index) => (
                                <div key={index} className="group h-full min-w-[300px] flex flex-col items-center justify-evenly px-8 border-x text-2xl backdrop-blur-sm">
                                    {content.graphics && (
                                        <div
                                            className="relative inline-block overflow-hidden rounded-xl px-8 py-3 text-sm font-medium bg-gray-900/30 transition-all duration-500 delay-100">
                                            <span className="ease absolute left-0 top-0 h-0 w-0 border-t-2 border-white/70 transition-all duration-200 group-hover:w-full"></span>
                                            <span className="ease absolute right-0 top-0 h-0 w-0 border-r-2 border-white/70 transition-all duration-200 group-hover:h-full"></span>
                                            <span className="ease absolute bottom-0 right-0 h-0 w-0 border-b-2 border-white/70 transition-all duration-200 group-hover:w-full"></span>
                                            <span className="ease absolute bottom-0 left-0 h-0 w-0 border-l-2 border-white/70 transition-all duration-200 group-hover:h-full"></span>
