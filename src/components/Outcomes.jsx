  ...
  return (
    <>
      {window.innerWidth >= 768 && (
        <section
          className="flex w-[80vw] min-w-[700px] mx-auto min-h-screen bg-gray-950/40 text-gray-300 rounded-lg overflow-hidden border shadow-md"
          ref={container}
        >
          <div
            className="w-1/2 h-screen flex items-center justify-center border-r-2 text-7xl backdrop-blur-md"
            ref={text}
          >
            Outcomes
          </div>
          <div className="w-1/2 my-[100px]">
            {data.map((content, index) => (
              <div
                key={index}
                className={`min-h-[300px] flex items-center justify-center content-div py-4 px-8 border-y text-2xl backdrop-blur-sm`}
              >
                {content}
              </div>
            ))}
          ...
          <div className="w-full px-[100px] flex h-[calc(100vh-200px)] overflow-x-hidden ">
            <div ref={horText} className="flex h-full">
              {data.map((content, index) => (
                <div
                  key={index}
                  className="h-full min-w-[300px] flex items-center justify-center px-8 border-x text-2xl backdrop-blur-sm"
                >
                  {content}
                </div>
              ))}
